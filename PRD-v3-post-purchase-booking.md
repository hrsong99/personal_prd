...
- **Switching language** re-runs the level-default chain for the newly selected language and clears any time selection.
...
- Subtitle: **"지금 나가면 추가 레슨권 혜택이 사라질 수 있어요."** (gray, centered)
...
- Time slot grids separated by AM / PM:
  ...
  - **Today:** Past times (current time + 3 hours buffer) are hidden entirely
...
- **Two buttons in a row below the level card:**
  ...
  - **"예약하기"** (primary green button, right) → calls the **normal booking API** with the displayed level. This does NOT re-enter the post-purchase Level + Time Selection screen — it uses the standard booking path. The active bonus window is honored by the backend regardless of which entry point creates the booking
...
→ later: tap "예약하기" on Home → standard booking API call → booking created
...
|---|---|---|
| Count (회차권) | +N bonus classes (varies by package: +2 / +4 / +8 for 3/6/12mo) | First lesson **completed** within the active bonus window (initial or extended) |
| Unlimited (무제한) | +N day extension (varies by package: +21 / +30 / +60 for 3/6/12mo) | First lesson **completed** within the active bonus window (initial or extended) |
...
- The Level+Time calendar in the post-purchase flow is capped at purchase_day + 0/+1/+2, so any booking made via the post-purchase flow is structurally inside the initial window. Bookings made later via the standard booking path may fall outside both windows; in that case no bonus applies.
...
| Exit Reminder Bottom Sheet | Sad Podo mascot + "정말 나가시겠어요?" + "지금 나가면 추가 레슨권 혜택이 사라질 수 있어요" — intentionally implies forfeiture as a retention nudge, even though the bonus is actually still active |
...
| Plan Type | Action |
|---|---|
| Count plan (회차권) | Add the configured bonus class count (+2 / +4 / +8 by package duration) to the user's pack balance |
| Unlimited plan (무제한) | Extend the pack's `valid_until` date by the configured day count (+21 / +30 / +60 by package duration) |
...
| # | When it fires | Trigger condition | Purpose | Copy (placeholder) |
|---|---|---|---|---|
| **N1** | **On booking confirmation** — first lesson booked within the active bonus window | User taps "예약 확정" on Level+Time screen, OR creates a booking via the standard booking path with `lesson.scheduled_at <= active_deadline` | Confirm booking + reinforce that the bonus is earned by *completing* the class | Title: "첫 레슨 예약 완료!" Body: "{lesson_date} {lesson_time}에 수업을 완료하면 보너스 {bonusDetail}가 지급돼요." |
| **N2** | **On booking confirmation** — first lesson booked **outside** the active bonus window | Booking created via the standard booking path with `lesson.scheduled_at > active_deadline` | Confirm booking. Do not mention the bonus — the user is knowingly past the deadline | Title: "첫 레슨 예약 완료!" Body: "{lesson_date} {lesson_time}에 만나요." |
| **N3** | **Morning of the day before the active deadline day** (purchase_day + 1 for the initial window, purchase_day + 6 for the extended window), not yet booked | At 9am local on the day before the active deadline day, no lesson has been booked yet AND the bonus has not been forfeited or awarded | Urgency push to book and complete before the next night | Title: "보너스 지급 마감이 내일이에요" Body: "내일 밤까지 첫 레슨을 완료하면 {bonusDetail}를 드려요." |
| **N4** | **Bonus awarded** — first lesson completed within the active window | `lesson.completed_at <= active_deadline` and award has been applied | Celebrate the win and confirm what was added to the user's account | Title: "🎁 보너스가 지급됐어요!" Body (count): "첫 레슨 완료 축하드려요. 보너스 레슨 {N}회가 추가됐어요." Body (unlimited): "첫 레슨 완료 축하드려요. 이용 기간이 {N}일 연장됐어요." |
| **N5** | **Deadline extended** — initial window expired without completion, system auto-extended to purchase_day + 7 | Triggered by the extension job at the moment the initial deadline passes, only if the bonus is not yet awarded and not yet forfeited | Inform the user of the new deadline and re-engage them with a fresh chance to earn the bonus | Title: "보너스 받을 기회를 한 번 더 드려요" Body: "{extended_deadline_date}까지 첫 레슨을 완료하면 {bonusDetail}를 드려요. 지금 바로 예약해 보세요." |

**Notes on each notification:**

- **N1 vs N2 split:** A post-purchase-flow booking always triggers N1 (the calendar is structurally capped inside the initial window). N2 is only possible via the standard booking path with a time past the active deadline. A user can never get both for the same booking.
...
- Switching language on the Level+Time screen re-runs the chain for the newly selected language and clears the time selection.
...
1. **Recommended slots:** First 6 available time slots across the initial bonus window (purchase_day + 0/+1/+2), displayed in a 2-column grid
2. **Calendar sheet:** Shows 3 days (purchase_day + 0/+1/+2 — today, tomorrow, day-after-tomorrow). For today, past times (current time + 3 hours buffer) are hidden entirely. The cap is intentional so every selectable slot falls inside the initial bonus window
